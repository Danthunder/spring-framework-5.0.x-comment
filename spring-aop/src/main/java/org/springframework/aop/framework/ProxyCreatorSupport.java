...
public class ProxyCreatorSupport extends AdvisedSupport {
	 ...
	 *
	 * 在spring底层，在创建动态代理对象时，是通过AopProxy接口来创建，创建代理对象的方法是
	 * createAopProxy()，该方法返回AopProxy类型，AopProxy是一个接口，有三个实现类（两种
	 * 技术---Cglib和Jdk Dynamic Proxy）分别是：该方法返回哪种AopProxy的实现类对象就代表
	 * 使用的哪一种动态代理技术！
	 * 	1. CglibAopProxy
