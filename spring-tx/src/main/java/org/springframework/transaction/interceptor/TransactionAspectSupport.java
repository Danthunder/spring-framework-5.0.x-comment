...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	 ...
	 * 2.执行InvocationCallback的proceedWithInvocation()：
	 * 		InvocationCallback是父类的内部回调接口，子类中实现该接口供父类调用，
	 * 		子类TransactionInterceptor中invocation.proceed()。回调方法执行
	 * 3.异常回滚completeTransactionAfterThrowing(txInfo, ex)，正常提交commitTransactionAfterReturning(txInfo)
	 * 4.清空当前事务信息cleanupTransactionInfo(txInfo)
	 */
	@Nullable
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// 获取 TransactionAttributeSource，后面利用解析器parse就利用该对象。
		// 该对象在初始化 TransactionInterceptor时就初始化
		// 如果transaction attribute为空,该方法就是非事务（非编程式事务）
		TransactionAttributeSource tas = getTransactionAttributeSource();

		// 从tas中获取事务属性txAttr，如果缓存有则直接取缓存，缓存没有则直接解析
		final TransactionAttribute txAttr = (tas != null ? tas.getTransactionAttribute(method, targetClass) : null);
		final PlatformTransactionManager tm = determineTransactionManager(txAttr);
		final String joinpointIdentification = methodIdentification(method, targetClass, txAttr);

		// 标准声明式事务：如果事务属性为空 或者 使用CallbackPreferringPlatformTransactionManager事务管理器
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		if (txAttr != null) {
			if (tm != null) {
				// 重要过程。根据事务属性获取事务TransactionStatus。这里主要处理事务的传播特性、
				// 隔离级别和是否只读。调用PlatformTransactionManager.getTransaction()
		...
		// 构造事务信息对象TransactionInfo，并绑定当前线程
	...
	protected TransactionInfo prepareTransactionInfo(@Nullable PlatformTransactionManager tm,
		...
		// 绑定到当前线程，ThreadLocal操作（将原来的 TransactionInfo 放在 oldTransactionInfo
		// 属性中保存，将新 TransactionInfo 放入当前 ThreadLocal 对象 transactionInfoHolder）
		txInfo.bindToThread();
		return txInfo;
	}
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		...
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			// 判断抛出的异常是否满足回滚条件，默认情况下是 RuntimeException or Error 的情况下回滚
