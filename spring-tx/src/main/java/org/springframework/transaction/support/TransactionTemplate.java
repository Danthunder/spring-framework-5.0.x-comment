 ...
 *
 *
 * TransactionTemplate实现了两个接口
 * TransactionOperations：用来执行事务的回调
 * InitializingBean：spring bean初始化流程中，属性装配完后，执行的方法
 * 		即实现afterPropertiesSet方法
 * afterPropertiesSet只是校验了事务管理器不为空，真正的核心代码是execute，execute主要步骤：
 * 1）getTransaction()获取事务
 * 2）doInTransaction()执行业务逻辑，这里就是用户自定义的业务代码。如果是没有返回值的，
 * 		就是doInTransactionWithoutResult()。
 * 3）commit()事务提交：调用AbstractPlatformTransactionManager的commit，或者
 *   rollbackOnException()异常回滚：调用AbstractPlatformTransactionManager的rollback()，事务提交回滚
 */
...
public class TransactionTemplate extends DefaultTransactionDefinition
	...
	public void afterPropertiesSet() {
		// afterPropertiesSet只是校验了事务管理器不为空
	...
	public <T> T execute(TransactionCallback<T> action) throws TransactionException {
		...
		// 内部封装好的事务管理器CallbackPreferringPlatformTransactionManager
