...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected Object doGetTransaction() {
		// 创建一个 DataSourceTransactionObject 事务对象
		...
		/**
		 * 从当前线程中获取已缓存的数据库连接信息（若存在）。注意从缓存 resources:ThreadLocal<Map<Object, Object>>
		 * 中获取，其保存了数据源和连接的map信息，key=DataSource，value=ConnectionHolder。这个
		 * ThreadLocal对象resources是实现事务的核心之一，即要保证同一个线程调用的不同方法在同一个
		 * 事务中，前提必须是同一个数据库连接 connection，否则无法实现事务
 		 */
		ConnectionHolder conHolder =
				(ConnectionHolder) TransactionSynchronizationManager.getResource(obtainDataSource());

		// 将连接信息设置到事务对象中，false 表示这个连接不是新创建的
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			...
			if (!txObject.hasConnectionHolder() ||
				...
				// 如果没有连接，则从数据库连接池中获取新连接Connection对象
				Connection newCon = obtainDataSource().getConnection();
				if (logger.isDebugEnabled()) {
					logger.debug("Acquired Connection [" + newCon + "] for JDBC transaction");
				}
				// 重置一个新的、可用的 connectionHolder
			...
			// 设置是否只读和隔离级别（如果没有手动设置隔离级别，就是用数据库默认隔离级别；否则将con的隔离级别重新调整并返回）
			Integer previousIsolationLevel = DataSourceUtils.prepareConnectionForTransaction(con, definition);
			// 同样在事务对象DataSourceTransactionObject中，标记事务隔离级别
			txObject.setPreviousIsolationLevel(previousIsolationLevel);

			// 如果是自动提交（auto commit）切换到手动提交，事务交由Spring管理
			...
			if (con.getAutoCommit()) {
				...
				con.setAutoCommit(false); // 关闭数据库事务的自动提交
			}
			...
			// 将新的连接，绑定connection持有者到当前线程，即事务同步管理器
			// TransactionSynchronizationManager中的 resources 对象
	...
	protected Object doSuspend(Object transaction) {
		...
		// 2.当前线程解绑datasource。即是ThreadLocal移除对应变量（TransactionSynchronizationManager类中定义的
	...
	protected void doCleanupAfterCompletion(Object transaction) {
		...
		try {
			if (txObject.isMustRestoreAutoCommit()) {
				con.setAutoCommit(true); // 将连接还原到自动提交事务的状态
			}
			// 重置连接的属性包括隔离级别、readOnly属性等
