...
class ConfigurationClassEnhancer {
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		// 判断是否被代理过
		// 即判断是否实现了EnhancedConfiguration接口
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		/*
		 * 增强接口，为什么？为了为代理类添加beanFactory的属性$$beanFactory
		 * EnhancedConfiguration接口继承了BeanFactoryAware接口，并最终继承Aware接口
		 */
		...
		设置此变量的值为当前Context中的beanFactory，这样一来，我们这个Cglib代理的对象就有了
		beanFactory。有了beanFactory就能获取对象，而不用去通过方法获取对象了，因为通过方法获
		得对象不能控制此过程
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			// 通过enhancedConfigInstance中，Cglib生成的成员变量$$beanFactory获得beanFactory。
			...
			// 判断方法是否是第一次调用beanMethod（是否需要new instance）
			// 即当前执行方法（invokedMethod）和beanMethod是否相同，
			// 若相同，表示是第一次调用beanMethod，执行父类（目标对象）的方法，invokeSuper()
			// 若不同，表示非第一次调用beanMethod，则执行resolveBeanReference()
