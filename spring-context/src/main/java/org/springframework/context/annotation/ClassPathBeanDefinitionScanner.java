		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				...
				if (candidate instanceof AbstractBeanDefinition) {
					...
					// 因为所有Spring扫描的类都是ScannedGenericBeanDefinition类型的（在上面
					// findCandidateComponents()），该类型最终继承AbstractBeanDefinition
					// 则为它设置默认值，比如lazy、init、destroy
					postProcessBeanDefinition((AbstractBeanDefinition) candidate, beanName);
				}
				if (candidate instanceof AnnotatedBeanDefinition) {
					// 检查并处理常用的注解
					// 这里的处理主要是指把常用注解的值设置到AnnotatedBeanDefinition当中
					// 当然前提是这个类必须是AnnotatedBeanDefinition类型的，即加了注解的类
				...
				if (checkCandidate(beanName, candidate)) {
					...
					// 将扫描并转化为definitionHolder（beanDefinition的封装）注册到
