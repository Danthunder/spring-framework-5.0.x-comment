...
abstract class ConfigurationClassUtils {
	...
	public static boolean checkConfigurationClassCandidate(BeanDefinition beanDef, MetadataReaderFactory metadataReaderFactory) {
		...
		if (beanDef instanceof AnnotatedBeanDefinition &&
			...
			// 如果BeanDefinition是AnnotatedBeanDefinition的实例，并且className和BeanDefinition中
			// 的元数据的类名相同，则直接从BeanDefinition获取metadata
		...
		// 判断当前beanDefinition中存在的类是不是加了@Configuration注解
		if (isFullConfigurationCandidate(metadata)) {
			// 如果存在@Configuration注解，则为BeanDefinition设置ConfigurationClass属性为full
			beanDef.setAttribute(CONFIGURATION_CLASS_ATTRIBUTE, CONFIGURATION_CLASS_FULL);
		}
		else if (isLiteConfigurationCandidate(metadata)) {
			// 如果不存在@Configuration注解，则为BeanDefinition设置ConfigurationClass属性为lite
